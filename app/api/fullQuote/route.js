...
function validate(body) {
  ...
  const length = (body?.length || "").toString().trim();
  const footage = (body?.footage || "").toString().trim();
  const petsChildren = (body?.petsChildren || "").toString().trim();
  const proCleaned = (body?.proCleaned || "").toString().trim();
  ...
  if (!address || address.length < 2) errs.push("address");
  if (!length || length.length < 2) errs.push("length");
  if (!footage || footage.length < 2) errs.push("footage");
  if (!petsChildren || petsChildren.length < 2) errs.push("petsChildren");
  if (!proCleaned || proCleaned.length < 2) errs.push("proCleaned");
  ...
  try {
    ...
    await resend.emails.send({
      ...
      text:
        ...
        `Email: ${email}\n` +
        `Address: ${address}\n` +
        `Length in Home: ${length}\n` +
        `Square Footage: ${footage}\n` +
        `Pets or Children: ${petsChildren}\n` +
        `Pro Cleaned: ${proCleaned}\n\n` +
        `${message}\n\n` +
        `Source: ${sourceUrl || "-"}\n` +
        `IP: ${ip}`,
    });
